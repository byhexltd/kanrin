...
- [x] **17.1.1** Audit every place where authenticated and unauthenticated connections diverge; document each divergence — `DESIGN-EVASION.md` §3a, eight tagged divergences `D1`–`D8` against the Phase 16 baseline, each mapped to the task that closes it. They collapse into two root causes: the port does not actually host a web server (`D1`, `D2`, `D6`), and authentication is a **branch taken before the response** rather than something discovered inside a session that was going to be served anyway (`D3`, `D4`, `D5`). The second is exactly the flaw **T3** used to break Reality.
- [x] **17.1.2** Unify TLS termination so one implementation handles both — there was never a second TLS stack to unify (unlike Reality, which is what **T3** exploited); the divergence was that the single stack then spoke no HTTP at all despite advertising `h2`/`http/1.1` (`D6`). Closed by making the one rustls terminator hand every connection to the one HTTP front door. `D7` (matching the claimed origin's TLS parameters) remains open and is tracked under 17.1.8.
- [x] **17.1.3** Serve real content from the same process and code path — `ServerConfig::origin()` resolves one content source used by every connection. There is deliberately no way to configure "this content for probers, that content for clients", since such a setting would be precisely the divergence 17.1 exists to remove.
- [x] **17.1.4** One HTTP response path (headers, static files) shared by all clients — new `kanrin-server/src/http.rs`, hand-rolled rather than delegated to a framework: 17.1.11 needs byte-exact responses, and a framework's header order, casing and default set fingerprint *the framework* rather than the site being claimed. Invariance is structural — a single `RequestHead::parse`, so the two cases cannot be parsed by different code or rejected by different rules. Includes a real keep-alive loop, because closing after one response while the claimed origin does not is detectable without reading any content. 19 tests.
- [x] **17.1.5** Optional upstream reverse-proxy target, same path for everyone — `kanrin-server/src/origin.rs`. The client's request head is forwarded **verbatim** (`RequestHead::raw`) and the origin's response returned unchanged, so the bytes are not *like* the origin's, they *are* the origin's — header order, `Date` format, `Server` banner and error pages included. A dead upstream yields a plausible 502 rather than silence (`D1`). 8 tests, one asserting header order survives the round trip.
- [x] **17.1.6** Discover authentication inside an already-established session, never as a branch that selects a different stack — `frontdoor::Admitter`. Every connection is served as HTTP *first*; the authenticator (an ordinary-looking `sid` cookie, HMAC over timestamp+nonce) is examined only after the response has been produced, and changes nothing about it. `admit()` returns `Web` for every failure mode — absent, malformed, expired, wrong password, replayed — so there is no "rejected" state to observe. Single-use nonces mean a captured cookie is as inert as a wrong one. Closes `D3`, `D5`.
- [x] **17.1.7** Normalize response timing so the auth outcome is not observable — `frontdoor::ResponseTimer`. A floor (25 ms) plus jitter (15 ms), started before any work and awaited just before the response, so it *absorbs* the extra work the authenticated path does (registry insert, forwarder registration) rather than adding to it — `D4`. Jitter matters independently: a perfectly constant response time is its own signature. Tests cover both directions, including that a 40 ms response is not then delayed further.
